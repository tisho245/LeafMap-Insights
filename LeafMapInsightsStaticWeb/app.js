 ...
 * Конфигурирайте API_BASE_URL според сървъра. За локално: първо опитайте http://localhost:5202
...
async function loadTrees() {
  ...
  try {
    ...
    if (Array.isArray(trees) && trees.length) {
      listEl.innerHTML = trees.map(renderTreeCard).join('');
    } else {
      listEl.innerHTML = '<p class="empty">Няма регистрирани дървета.</p>';
    }
  } catch (e) {
    if (loadingEl) loadingEl.hidden = true;
    if (errorEl) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (formLogin) formLogin.addEventListener('submit', async e => {
    ...
    try {
      ...
      if (errEl) { errEl.textContent = err.message || 'Грешка при вход.'; errEl.hidden = false; }
    }
  });
